...
**Checkpoint 39 — GAME-022: implemented and covered by automated tests.**
...
Not implemented yet (intentionally): automatic resupply, multi-patrol contact arbitration and avoidance clearance, pursuit route replanning, cross-session/server-persisted player knowledge, rewards and expedition persistence, the production physical player map and full terrain fog of war, server/database, tactical combat, and autonomous neural NPC / City / Species agents.
...
This total contains 341 simulation/UI tests and 5 tooling regression tests.

GitHub Actions installs exact dependencies, compiles `sim-core`, type-checks the browser UI, and runs all tests for every pull request to `main`. See `docs/DEVELOPMENT_WORKFLOW.md` for the pre-MVP process and rollback rules, and `docs/CHECKPOINT_39.md` for GAME-022 details.
...
Then open `http://127.0.0.1:4173`. The world map is intentionally a developer overlay: it shows exact coordinates, hidden static objects, monster radii, patrol routes, the editable four-segment caravan route, supplies, timeline, doctrines, outcomes and Power/FLEE resolution. The separate player-facing map starts as unexplored darkness. Select `STOP`, press `DEV: маршрут к цели`, and advance time: only the actually travelled path cuts a transparent corridor with a physical radius of 300 m. Planned future legs are never drawn, moving the DEV slider backwards does not erase retained visibility, and earlier expedition corridors remain. The center line and confirmed knowledge markers stay visible above the fog. Independent origin cities still use separate charts. After an authoritative city arrival, the reached city appears as a confirmed relative landmark only on the expedition's origin-city chart. `DEV: возврат в пути` prepares GAME-017; two more clicks advance to the moving 50% decision and successful origin-city re-entry. `DEV: возврат из STOP` prepares GAME-018 with a six-hour discovery wait and idle-only consumption; its next two clicks advance to the two-hour 50% boundary and then the same authoritative return. For GAME-020, keep `AVOID` selected and press `DEV: маршрут на перехват`; the next two clicks advance to the exact 1000 m warning/decision and then the end of the verified one-waypoint detour. The journal contains `danger-detected` followed by `danger-doctrine-decision` but no selected-patrol contact. Select `CONTINUE` and prepare another intercept to retain the original route and its 500 m contact. For GAME-021, keep `STOP` and `AVOID` selected and press `DEV: опасность во время STOP`; the next two clicks advance to the exact idle warning and then the end of the detour. At the warning, world time has advanced but route time remains pinned, the journal records `danger-avoidance` resume provenance, and only the remaining part of the six-hour wait is cancelled. Repeat with `CONTINUE` to preserve the full wait and later 500 m contact. GAME-022 is always visible in the danger panel: it reports the first 1000 m warning across both generated patrols, including the winning monster ID, exact time and moving/STOP activity. The doctrine control below it remains an explicitly selected-patrol QA path until GAME-023 adds multi-patrol clearance. The local contact inset still draws both concentric server-truth boundaries. Discoveries, tracks, city landmarks and session fog reset together on a different seed, page reload or the clear button. Existing arrival, idle-contact and Power/FLEE DEV presets remain available. Stop the server with `Ctrl+C`.
...
The next small checkpoint is GAME-023: execute moving `AVOID | CONTINUE` for the selected first warning and validate every accepted AVOID route continuously against all patrols; scheduled-STOP composition remains a later slice.
